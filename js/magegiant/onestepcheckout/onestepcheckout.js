var MagegiantOnestepcheckoutCore = {
    initialize: function () {
        this.updater = MagegiantOnestepcheckoutCoreUpdater;
    ...
    updateNumbers: function () {
        var currentField = 1;
        $$('.giant-onestepcheckout-number').each(function (el) {
            var match = el.className.match(/giant-onestepcheckout-number-[0-9]/)
            if (!match || !match[0]) {
                return;
            }
            el.removeClassName(match[0]);
            el.addClassName('giant-onestepcheckout-number-' + currentField);
...
var MagegiantOnestepcheckoutCoreUpdater = {
    ...
    addActionBlocksToQueue: function (action) {
        ...
        this.map[action].each(function (blockName) {
            ...
            if (me.queue[blockName] === 0) {
                ...
                MagegiantOnestepcheckoutCore.addLoaderOnBlock(targetBlockForAddLoader, me.loaderConfig);
    ...
    removeActionBlocksFromQueue: function (action, response) {
        ...
        this.map[action].each(function (blockName) {
            ...
            if (me.queue[blockName] === 0) {
                ...
                MagegiantOnestepcheckoutCore.removeLoaderFromBlock(me.blocks[blockName], me.loaderConfig);
...
MagegiantOnestepcheckoutCore.initialize();
MagegiantOnestepcheckoutHelperTimer = Class.create();
MagegiantOnestepcheckoutHelperTimer.prototype = {
    ...
    onTimerRedirectElClick: function (e) {
        ...
        //add loader css
        var cssClassName = MagegiantOnestepcheckoutCore.getCssLoaderClassForBlock(this.container, this.overlayConfig);
        if (cssClassName) {
...
MagegiantOnestepcheckoutUIPopup = Class.create();
MagegiantOnestepcheckoutUIPopup.prototype = {
    initialize: function (config) {
    ...
    initObservers: function () {
        ...
        if (this.buttons.accept && this.buttons.accept.el) {
    ...
    _resizePopup: function () {
        ...
        var contentHeight = this.container.getHeight() - parseInt(this.container.getStyle('padding-top')) -
            parseInt(this.container.getStyle('padding-bottom')) - parseInt(this.acceptContainer ? this.acceptContainer.getHeight() : 0);
