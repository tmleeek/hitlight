...
class Celigo_Celigoconnector_Helper_Extensionconflict extends Mage_Core_Helper_Abstract {
  ...
  <tbody>';
        ...
        if (count($conflicts) > 0) {
            ...
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('celigoconnector')->__($title));
    ...
    private function getConfigFilesList() {
        ...
        foreach ($locations as $location) {
            ...
            while ($namespaceName = readdir($poolDir)) {
                if (!$this->directoryIsValid($namespaceName)) continue;
                ...
                while ($moduleName = readdir($namespaceDir)) {
                    if (!$this->directoryIsValid($moduleName)) continue;
                    $modulePath = $namespacePath . $moduleName . '/';
                    $configXmlPath = $modulePath . 'etc/config.xml';
                    if (file_exists($configXmlPath)) $retour[] = $configXmlPath;
                }
    ...
    private function getRewriteForFile($configFilePath, $results) {
        try {
            ...
            if (!$domDocument) {
                ...
                return $results;
            } elseif (!$domDocument->documentElement) {
            ...
            Mage::helper('celigoconnector/celigologger')->error($e->getMessage() , self::LOG_FILENAME);
        }
