...
class Celigo_Celigoconnector_Model_System_Config_Backend_Password extends Mage_Core_Model_Config_Data {
    const XML_PATH_NETSUITE_USER_PASSWORD = 'celigoconnector/nsdetails/nspassword';
    ...
    protected function _beforeSave() {
        ...
        if (trim($value) == '' || trim($password) == '') {
            throw new Exception(Mage::helper('celigoconnector')->__(self::PASSWORDS_REQUIRED_MSG));
        }
        if ($value != $password) {
            throw new Exception(Mage::helper('celigoconnector')->__(self::PASSWORDS_MISMATCH_MSG));
            $this->setValue('');
        ...
        $isactive = $this->getData('groups/magentoceligoconnector/fields/active/value');
        if ($isactive) {
            ...
            if ($nsemail != '' && $nspassword != '' && $nsaccount != '' && $nsrole != '' && $nsenvironment != '') {
                $result = Mage::helper('celigoconnector')->validateNetsuiteCredentials($nsemail, $nspassword, $nsaccount, $nsrole, $nsenvironment, '', '');
            }
            if ($result === true) {
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('celigoconnector')->__(self::VALID_NETSUITE_CREDENTIAL_MSG));
            } else {
                throw new Exception(Mage::helper('celigoconnector')->__($result));
            }
        }
     ...
     *
     * @return Celigo_Celigoconnector_Model_System_Config_Backend_Password
     */
    public function _afterSave() {
        $value = $this->getValue();
        if ($value == "") {
            Mage::getConfig()->saveConfig(self::XML_PATH_NETSUITE_USER_PASSWORD, '', $this->getScope() , $this->getScopeId());
        }
