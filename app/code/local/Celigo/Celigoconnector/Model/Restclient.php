...
class Celigo_Celigoconnector_Model_Restclient extends Mage_Core_Model_Abstract {
    public $options;
    ...
    public function deleteit($url, $parameters = array() , $headers = array()) {
    ...
    private function execute($url, $method = 'GET', $parameters = array() , $headers = array()) {
        ...
        while ($isAllowed) {
            ...
            if ($client->options['username'] && $client->options['password']) $curlopt[CURLOPT_USERPWD] = sprintf("%s:%s", $client->options['username'], $client->options['password']);
            ...
            if (strtoupper($method) == 'POST') {
                ...
                $curlopt[CURLOPT_POSTFIELDS] = $parameters['body']; //$client->format_query($parameters);
                
            } elseif (strtoupper($method) == 'PUT') {
                //$curlopt[CURLOPT_POST] = TRUE;
                $curlopt[CURLOPT_CUSTOMREQUEST] = 'PUT';
                $curlopt[CURLOPT_POSTFIELDS] = $parameters['body'];
            ...
            if ($client->options['base_url']) {
                if ($client->url != '') {
                    if ($client->url[0] != '/' || substr($client->options['base_url'], -1) != '/') $client->url = '/' . $client->url;
                }
            ...
            if ($client->info->http_code == $tryOnErrCode && $counter < $numOfMaxTries) {
                $isAllowed = true;
                sleep($sleepTimeInSecons);
            } else {
                $isAllowed = false;
            }
            $counter++;
