...
class Celigo_Celigoconnector_Model_Product_Api extends Mage_Catalog_Model_Product_Api {
    /**
        ...
        */
        ...
        if ($type == 'bundle') {
            ...
            try {
                    ...
                    foreach ($childItems as $childItem) {
                        if (is_array($childItem) && count($childItem) > 0) {
                            ...
                            foreach ($childItem as $key => $value) {
                                ...
                                $bundle_items[] = array(
                                    "id" => $bundle_item->getId() ,
                                    "sku" => $bundle_item->getSku()
                                );
                            }
                        }
                    }
                ...
                Mage::helper('celigoconnector/celigologger')->error('errormsg="'.$e->getMessage().'"'.' productId="' . $productId.'"', LOG_FILENAME);
            }
        }
    ...
    public function create($type, $set, $sku, $productData, $store = null) {
        if (!$type || !$set || !$sku) {
            Mage::helper('celigoconnector/celigologger')->error('errormsg="'."type :$type, set :$set,sku :$sku ". '" api="product.create" error="data_invalid"' , LOG_FILENAME);
            $this->_fault('data_invalid');
        }
        if (!in_array($type, array_keys(Mage::getModel('catalog/product_type')->getOptionArray()))) {
            Mage::helper('celigoconnector/celigologger')->error('errormsg="'."type :$type does not exist.". '" api="product.create" error="data_invalid"' , LOG_FILENAME);
        ...
        if (is_null($attributeSet->getId())) {
            Mage::helper('celigoconnector/celigologger')->error('errormsg="'."attributeSet :$set does not exist.".'" api="product.create" error="data_invalid"' , LOG_FILENAME);
        ...
        if (Mage::getModel('catalog/product')->getResource()->getTypeId() != $attributeSet->getEntityTypeId()) {
            Mage::helper('celigoconnector/celigologger')->error('errormsg="'."Attribute Set Id :$set is not valid.".'" api="product.create" error="data_invalid"' , LOG_FILENAME);
            return 'Attribute Set Id ' . $set . ' is not valid';
        }
        /** @var $product Mage_Catalog_Model_Product */
        $product = Mage::getModel('catalog/product');
        $product->setStoreId($this->_getStoreId($store))->setAttributeSetId($set)->setTypeId($type)->setSku($sku);
        if (isset($productData['website_ids']) && is_array($productData['website_ids'])) {
            $product->setWebsiteIds($productData['website_ids']);
        }
        if ($type == Mage_Catalog_Model_Product_Type::TYPE_BUNDLE && !isset($productData['price_type'])) {
            $productData['price_type'] = 0; // 0 for Dynamic and 1 for Fixed
        }
        ...
        */
        if ($type == 'bundle') {
            if (isset($productData['bundle_options']) && is_array($productData['bundle_options'])) {
                ...
                foreach ($productData['bundle_options'] as $optionId => $otpionvalues) {
                    if (!isset($otpionvalues['type'])) $productData['bundle_options'][$optionId]['type'] = 'select';
                    if (!isset($otpionvalues['required'])) $productData['bundle_options'][$optionId]['required'] = 1;
                ...
                if (is_array($bundle_selections) && count($bundle_selections) > 0) {
                    if(!isset($productData['bundle_selections'])) {
                        $productData['bundle_selections'] = $bundle_selections;
                    } elseif(!is_array($productData['bundle_selections'])) {
                        $productData['bundle_selections'] = $bundle_selections;
                    }
                }
            }
                    ...
                    foreach ($products as $indexId => $_product) {
                        if (isset($_product['product_id']) && $_product['product_id'] != '') {
                            ...
                            if (!in_array($_item->getTypeId() , $allowedTypes)) {
                                ...
                                Mage::helper('celigoconnector/celigologger')->error('errormsg="'.$errorMessage.'", record="Product" api="Product.update"', self::LOG_FILENAME);
                            ...
                            if (!isset($productData['bundle_selections'][$optionId][$indexId]['selection_qty'])) $productData['bundle_selections'][$optionId][$indexId]['selection_qty'] = 1;
                            if ($productsCount == 0 && !isset($productData['bundle_selections'][$optionId][$indexId]['is_default'])) $productData['bundle_selections'][$optionId][$indexId]['is_default'] = 1;
                            $productsCount++;
                        }
                    }
                }
        ...
        try {
            ...
            Mage::helper('celigoconnector/celigologger')->error('errormsg="'.$e->getMessage().'", record="product" api="product.create"', self::LOG_FILENAME);
    ...
    public function update($productId, $productData, $store = null, $identifierType = null) {
        $product = $this->_getProduct($productId, $store, $identifierType);
        if (!$product->getId()) {
            Mage::helper('celigoconnector/celigologger')->error('errormsg="No Product found with id "'.$productId.'", record="Product" api="Product.update"', self::LOG_FILENAME);
        ...
        if ($product->getTypeId() == Mage_Catalog_Model_Product_Type::TYPE_BUNDLE) {
            if (!isset($productData['price_type']) && $product->getPriceType() == '') {
                $productData['price_type'] = 0; // 0 for Dynamic and 1 for Fixed
            }
        }
        ...
        */
        if ($product->getTypeId() == 'configurable') {
            if (isset($productData['configurable_products_data']) && is_array($productData['configurable_products_data'])) {
                ...
                $allChildItems = $this->getAllAvailableChildProducts($product, $cProducts);
                ...
                foreach ($productData['configurable_products_data'] as $key => $prodinfo) {
                    if (!is_array($prodinfo)) {
                        if (!in_array($prodinfo, $allChildItemIds)) {
                            Mage::helper('celigoconnector/celigologger')->error('errormsg="'."Invalid associated product id: " . $prodinfo.'", record="product" api="product.create"', self::LOG_FILENAME);
            ...
            if (isset($productData['configurable_attributes_data']) && is_array($productData['configurable_attributes_data'])) {
                ...
                foreach ($productData['configurable_attributes_data'] as $key => $data) {
                  ...
                  if(isset($data['configurable_product_optionpricing_data'])) {
                      foreach($data['configurable_product_optionpricing_data'] as $ok => $odata) {
                          $optionpricing_data[$data['attribute_id']][$odata['option_id']] = $odata;
                          if(false !== strpos($odata['option_price'], '%')){
                              $optionpricing_data[$data['attribute_id']][$odata['option_id']]['is_percent'] = 2;
                              $optionpricing_data[$data['attribute_id']][$odata['option_id']]['option_price'] = preg_replace('/[^0-9\.-]/', '', $odata['option_price']);
                          }else{
                              $optionpricing_data[$data['attribute_id']][$odata['option_id']]['is_percent'] = 0;
                          }
                      }
                  }
                }
                ...
                foreach($productData['configurable_attributes_data'] as $key => $ca_data) {
                  if (array_key_exists($ca_data['attribute_id'], $optionpricing_data)) {
                      foreach($ca_data['values'] as $okey => $option) {
                          if (array_key_exists($option['value_index'], $optionpricing_data[$ca_data['attribute_id']])) {
                              $productData['configurable_attributes_data'][$key]['values'][$okey]['pricing_value'] = $optionpricing_data[$ca_data['attribute_id']][$option['value_index']]['option_price'];
                              $productData['configurable_attributes_data'][$key]['values'][$okey]['is_percent'] = $optionpricing_data[$ca_data['attribute_id']][$option['value_index']]['is_percent'];
                          }
                      }
                  }
                }
        ...
        */
        ...
        try {
            ...
            Mage::helper('celigoconnector/celigologger')->error('errormsg="'.$e->getMessage().'", record="Product" api="Product.update"', self::LOG_FILENAME);
            $this->_fault('data_invalid', $e->getMessage());
        }

        return true;
    }
    private function getAllAvailableChildProducts($product, $cProducts = array()) {
        ...
        $collection = $product->getCollection()->addAttributeToSelect('name')->addAttributeToSelect('sku')->addAttributeToSelect('attribute_set_id')->addAttributeToSelect('type_id')->addAttributeToSelect('price')->addFieldToFilter('attribute_set_id', $product->getAttributeSetId())->addFieldToFilter('type_id', $allowProductTypes)->addFilterByRequiredOptions()->addFieldToFilter('entity_id', array(
            "in" => $cProducts
    ...
    //we dont need this function as Magento always gives saved instance of a helper class
    //rather than creating a separate class each time called!
}
