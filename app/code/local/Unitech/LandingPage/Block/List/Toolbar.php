 ...
 */
class Unitech_LandingPage_Block_List_Toolbar extends Mage_Core_Block_Template
{
    /**
     * Products collection
     *
     ...
     * @var array
     */
    protected $_defaultAvailableLimit  = array(30=>30,60=>60,90=>90);
    ...
    {
        parent::_construct();
        $this->_orderField  = $this->_getConfig()->getLandingPageListDefaultSortBy();

        $this->_availableOrder = $this->_getConfig()->getAttributeUsedForSortByArray();

        switch (Mage::getStoreConfig('unitech_landingpage/frontend/list_mode')) {
            case 'grid':
            default:
                $this->_availableMode = array('grid' => $this->__('Grid'));
                break;
        }
        $this->setTemplate('unitech/landingpage/list/toolbar.phtml');
    }
    ...
    {
        $session = Mage::getSingleton('unitech_landingpage/session');
    ...
    /**
     * Return landing page collection instance
     *
    ...
    /**
     * Get grit landing page sort order field
     *
     * @return string
     */
    public function getCurrentOrder()
    {
        ...
        if ($order && isset($orders[$order])) {
            if ($order == $defaultOrder) {
                Mage::getSingleton('unitech_landingpage/session')->unsSortOrder();
            } else {
                $this->_memorizeParam('sort_order', $order);
            }
        } else {
            $order = Mage::getSingleton('unitech_landingpage/session')->getSortOrder();
        }
    ...
    public function getCurrentDirection()
    {
        ...
        if ($dir && in_array($dir, $directions)) {
            if ($dir == $this->_direction) {
                Mage::getSingleton('unitech_landingpage/session')->unsSortDirection();
            } else {
                $this->_memorizeParam('sort_direction', $dir);
            }
        } else {
            $dir = Mage::getSingleton('unitech_landingpage/session')->getSortDirection();
        }
        // validate direction
        if (!$dir || !in_array($dir, $directions)) {
            $dir = $this->_direction;
     ...
     *
     * @param string $order
     * @param Unitech_LandingPage_Block_List_Toolbar
     */
    ...
    public function getCurrentMode()
    {
        ...
        if ($mode) {
            if ($mode == $defaultMode) {
                Mage::getSingleton('unitech_landingpage/session')->unsDisplayMode();
            } else {
                $this->_memorizeParam('display_mode', $mode);
            }
        } else {
            $mode = Mage::getSingleton('unitech_landingpage/session')->getDisplayMode();
        }
    ...
    public function getDefaultPerPageValue()
    {
        ...
        return Mage::getStoreConfig('unitech_landingpage/frontend/grid_per_page');
    }
    ...
    protected function _getAvailableLimit($mode)
    {
        ...
        $perPageConfigKey = 'unitech_landingpage/frontend/' . $mode . '_per_page_values';
        $perPageValues = (string)Mage::getStoreConfig($perPageConfigKey);
        $perPageValues = explode(',', $perPageValues);
        $perPageValues = array_combine($perPageValues, $perPageValues);
        if (Mage::getStoreConfigFlag('unitech_landingpage/frontend/list_allow_all')) {
    ...
    /**
     * Get specified landing page limit display per page
     *
     * @return string
     */
    public function getLimit()
    {
        ...
        if ($limit && isset($limits[$limit])) {
            if ($limit == $defaultLimit) {
                Mage::getSingleton('unitech_landingpage/session')->unsLimitPage();
            } else {
                $this->_memorizeParam('limit_page', $limit);
            }
        } else {
            $limit = Mage::getSingleton('unitech_landingpage/session')->getLimitPage();
        }
        if (!$limit || !isset($limits[$limit])) {
            $limit = $defaultLimit;
        }
    ...
    {
        $pagerBlock = $this->getChild('landingpage_list_toolbar_pager');
