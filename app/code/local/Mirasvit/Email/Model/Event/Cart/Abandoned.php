...
class Mirasvit_Email_Model_Event_Cart_Abandoned extends Mirasvit_Email_Model_Event_Abstract
{
    ...
    protected function _check($eventCode, $from)
    {
        $events = $this->findEvents($eventCode, $from);

        foreach ($events as $event) {
            $key   = array();
            $key[] = $event['customer_email'];
            $key[] = $event['quote_id'];
    ...
    public function findEvents($eventCode, $timestamp)
    {
        ...
        $updatedAtFrom = date('Y-m-d H:i:s', $timestamp - 3600 * 2);
        $updatedAtTo   = date('Y-m-d H:i:s', Mage::getSingleton('core/date')->gmtTimestamp() - 5 * 60);
