...
*/
class Evince_Customattribute_Helper_Data extends Mage_Core_Helper_Abstract
{
    public function fields($fields = array())
    {
    	    	
        return Mage::app()->getLayout()->createBlock('customattribute/customer_fields')
    ...
    public function getAttributeImageUrl($optionId)
    {
        ...
        {
            return Mage::getBaseUrl('media') . '/' . 'customattribute' . '/' . 'images' . '/' . $optionId . '.jpg';
        }
        return '';
    }
    ...
    public function getAttributeFileUrl($fileName, $download = false, $front = false, $customerId = null)
    {
        ...
        if ($download) { // URL for download
            if (file_exists($fileDir . DIRECTORY_SEPARATOR . $fileName)) {
                if ($front) {
                    return Mage::getModel('core/url')->getUrl('customattributefront/attachment/download', array('customer' => $customerId, 'file' => Mage::helper('core')->urlEncode($fileName)));
                } else {
    ...
    {
        if ($asHash) {
            return array('text'           => Mage::helper('customattribute')->__('Text Field'),
                         'textarea'       => Mage::helper('customattribute')->__('Text Area'),
                         'date'           => Mage::helper('customattribute')->__('Date'),
                         'multiselect'    => Mage::helper('customattribute')->__('Multiple Select'),
                         'multiselectimg' => Mage::helper('customattribute')->__('Multiple Checkbox Select with Images'),
                         'select'         => Mage::helper('customattribute')->__('Dropdown'),
                         'boolean'        => Mage::helper('customattribute')->__('Yes/No'),
                         'selectimg'      => Mage::helper('customattribute')->__('Single Radio Select with Images'),
                         'selectgroup'    => Mage::helper('customattribute')->__('Customer Group Selector'),
                         'statictext'     => Mage::helper('customattribute')->__('Static Text'),
                         'file'           => Mage::helper('customattribute')->__('Single File Upload'),
                        );
        }
        return array(
            array(
                'value' => 'text',
                'label' => Mage::helper('customattribute')->__('Text Field')
            ),
            array(
                'value' => 'textarea',
                'label' => Mage::helper('customattribute')->__('Text Area')
            ),
            array(
                'value' => 'date',
                'label' => Mage::helper('customattribute')->__('Date')
            ),
            array(
                'value' => 'multiselect',
                'label' => Mage::helper('customattribute')->__('Multiple Select')
            ),
            array(
                'value' => 'multiselectimg',
                'label' => Mage::helper('customattribute')->__('Multiple Checkbox Select with Images')
            ),
            array(
                'value' => 'select',
                'label' => Mage::helper('customattribute')->__('Dropdown')
            ),
            array(
                'value' => 'boolean',
                'label' => Mage::helper('customattribute')->__('Yes/No')
            ),
            array(
                'value' => 'selectimg',
                'label' => Mage::helper('customattribute')->__('Single Radio Select with Images')
            ),
            array(
                'value' => 'selectgroup',
                'label' => Mage::helper('customattribute')->__('Customer Group Selector')
            ),
            array(
                'value' => 'statictext',
                'label' => Mage::helper('customattribute')->__('Static Text')
            ),
            array(
                'value' => 'file',
                'label' => Mage::helper('customattribute')->__('Single File Upload')
            ),
        );
    }
}