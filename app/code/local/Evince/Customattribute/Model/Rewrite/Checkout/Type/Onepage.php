<?php
class Evince_Customattribute_Model_Rewrite_Checkout_Type_Onepage extends Mage_Checkout_Model_Type_Onepage
{
    public function saveBilling($data, $customerAddressId)
    {
        if (isset($data['customattribute']))
        {
            ...
            foreach ($data['customattribute'] as $attributeCode => $attributeValue){
            ...
            if ($checkUnique) {
                foreach ($checkUnique as $attributeCode => $attributeLabel) {
                    //skip empty values
                    if (!$data['customattribute'][$attributeCode]) {
                        continue;
                    }
                    $customerCollection = Mage::getResourceModel('customer/customer_collection');
                    $customerCollection->addAttributeToFilter($attributeCode, array('eq' => $data['customattribute'][$attributeCode]));
                    ...
                    if ($customerCollection->getSize() > 0) {
                        $result = array(
                            'error'     => 1,
                            'message'   => Mage::helper('customattribute')->__('Please specify different value for "%s" attribute. Customer with such value already exists.', $attributeLabel),
                        );
                        return $result;
                    }
                }
            }
            Mage::getSingleton('checkout/session')->setAmcustomerattr($data['customattribute']);
        }
