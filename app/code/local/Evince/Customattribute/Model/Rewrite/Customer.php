...
*/
class Evince_Customattribute_Model_Rewrite_Customer extends Mage_Customer_Model_Customer
{
    ...
    protected function _beforeDelete()
    {
        ...
        foreach ($collection as $attribute) {
            if ($value = $this->getData($attribute->getAttributeCode())) {
                Mage::helper('customattribute')->deleteFile($value);
            }
        }
    ...
    protected function _afterSave()
    {
        if ($this->_fileAttributes) {
            $deleteFiles = Mage::app()->getRequest()->getPost('customattribute_delete');
            foreach ($this->_fileAttributes as $id => $attributeCode) {
                if (isset($_FILES['customattribute_' . $attributeCode]['error']) && UPLOAD_ERR_OK == $_FILES['customattribute_' . $attributeCode]['error']) {
                    try {
                        $fileName = $_FILES['customattribute_' . $attributeCode]['name'];
                        $uploader = new Varien_File_Uploader('customattribute_' . $attributeCode);
                        $uploader->setAllowRenameFiles(false);
                        $uploader->setFilesDispersion(false);
                        $destinationFolder = Mage::helper('customattribute')->getAttributeFileUrl($fileName);
                        ...
                        $e = new Mage_Customer_Exception(Mage::helper('customattribute')->__('An error occurred while saving the file: ' . $error->getMessage()));
                        if (method_exists($e, 'setMessage')) {
                            $e->setMessage(Mage::helper('customattribute')->__('An error occurred while saving the file: ' . $error));
                        }
                        throw $e;
                    }
                    if ($this->_savedFileNames[$attributeCode]) {
                        Mage::helper('customattribute')->deleteFile($this->_savedFileNames[$attributeCode]);
                    }
                }
                if ($deleteFiles && $deleteFiles[$attributeCode] && ($this->_savedFileNames[$attributeCode] === $deleteFiles[$attributeCode])) {
                    Mage::helper('customattribute')->deleteFile($deleteFiles[$attributeCode]);
                }
                unset($_FILES['customattribute_' . $attributeCode]);
            }
        }
        
        if(!$this->getOrigId() && !Mage::registry('customattribute_customer_registry_dispatched')) {
            Mage::register('customattribute_customer_registry_dispatched', true);
            Mage::dispatchEvent('customattribute_customer_registry', array('model' => $this)); 
        }
    ...
    protected function _beforeSave()
    {
        ...
        */
        $customerAttributes = Mage::app()->getRequest()->getPost('customattribute');
        ...
        if ($customerAttributes) {
            ...
            foreach ($customerAttributes as $attributeCode => $attributeValue) {
                ...
                if (in_array($attributeCode, $filesFields)) {
                    if (isset($_FILES['customattribute_' . $attributeCode]['error']) && UPLOAD_ERR_OK == $_FILES['customattribute_' . $attributeCode]['error']) { // check if uploaded new file
                        // correct filename
                        $temp = explode('.', $_FILES['customattribute_' . $attributeCode]['name']);
                        $ext = strtolower(array_pop($temp));
                        $fileName = Mage::helper('customattribute')->getCorrectFileName($temp[0]);
                        $f1 = Mage::helper('customattribute')->getFolderName($fileName[0]);
                        $f2 = Mage::helper('customattribute')->getFolderName($fileName[1]);
                        ...
                        $_FILES['customattribute_' . $attributeCode]['name'] = $attributeValue;
           ...
           } else if (Mage::helper('customattribute/group')->isAllowed()) {
               if ($this->getData(Mage::helper('customattribute/group')->getAttribute())) {
                   $this->setGroupId(Mage::helper('customattribute/group')->getGroupId());
               }
           }
        }
        ...
        if ($checkUnique) {
            foreach ($checkUnique as $attributeCode => $attributeLabel) {
                ...
                if ($customerCollection->getSize() > 0) {
                    $e = new Mage_Customer_Exception(Mage::helper('customattribute')->__('Please specify different value for `%s` attribute. Customer with such value already exists.', $attributeLabel));
                    if (method_exists($e, 'setMessage')) {
                        $e->setMessage(Mage::helper('customattribute')->__('Please specify different value for `%s` attribute. Customer with such value already exists.', $attributeLabel));
                    }
                    throw $e;
                }
            }
        ...
        if ($filesFields) {
            foreach ($filesFields as $attributeCode) {
                if (isset($_FILES['customattribute_' . $attributeCode]['error']) && UPLOAD_ERR_OK == $_FILES['customattribute_' . $attributeCode]['error']) {
                    // check file size
                    if ($filesRestrictions[$attributeCode]['size'] && ($filesRestrictions[$attributeCode]['size'] < $_FILES['customattribute_' . $attributeCode]['size'])) {
                        $fileErrors[] = Mage::helper('customattribute')->__('File size restriction: %d bytes', $filesRestrictions[$attributeCode]['size']);
                    }
                    // check file ext
                    if ($filesRestrictions[$attributeCode]['type']) {
                        ...
                        if (!in_array($ext, explode(',', $filesRestrictions[$attributeCode]['type']))) {
                            $fileErrors[] = Mage::helper('customattribute')->__('File ext restriction: %s', $filesRestrictions[$attributeCode]['type']);
                        }
                    }
                    // check type of file
                    if (substr_count($_FILES['customattribute_' . $attributeCode]['type'], '/') > 1) { // check double file type
                        $fileErrors[] = Mage::helper('customattribute')->__('Not supported type of file: %s', $_FILES['customattribute_' . $attributeCode]['type']);
                    }
                    // check content (MIME) type of file
                    if (Mage::getStoreConfig('customattribute/general/check_file_type') && !$this->checkContentType($ext, $_FILES['customattribute_' . $attributeCode]['type'])) {
                        $fileErrors[] = Mage::helper('customattribute')->__('Not supported content type of file: %s', $_FILES['customattribute_' . $attributeCode]['type']);
                    }
                    // check dimentions for image
                    if ($filesRestrictions[$attributeCode]['dimentions'] && (false !== strpos($_FILES['customattribute_' . $attributeCode]['type'], 'image/'))) {
                        $dimentions = explode('/', $filesRestrictions[$attributeCode]['dimentions']);
                        try {
                            $imageInfo = getimagesize($_FILES['customattribute_' . $attributeCode]['tmp_name']);
                        } catch (Exception $error) {
                            $e = new Mage_Customer_Exception(Mage::helper('customattribute')->__('System error: %s', $error));
                            if (method_exists($e, 'setMessage')) {
                                $e->setMessage(Mage::helper('customattribute')->__('System error: %s', $error));
                            }
                            throw $e;
                        }
                        $imageInfo = getimagesize($_FILES['customattribute_' . $attributeCode]['tmp_name']);
                        if (($imageInfo[0] > $dimentions[0]) || ($imageInfo[1] > $dimentions[1])) {
                            $fileErrors[] = Mage::helper('customattribute')->__('Image size restriction: %s pixels (width/height)', $filesRestrictions[$attributeCode]['dimentions']);
                        }
                    }
                    // errors
                    if ($fileErrors) {
                        $e = new Mage_Customer_Exception(Mage::helper('customattribute')->__('File error: %s', implode('; ', $fileErrors)));
                        if (method_exists($e, 'setMessage')) {
                            $e->setMessage(Mage::helper('customattribute')->__('File error: %s', implode('; ', $fileErrors)));
                        }
                        throw $e;
                    }
                }
    ...
    public function loadByEmail($customerEmail)
    {
        ...
        if (!Mage::getStoreConfig('customattribute/login/disable_email') || !Mage::getStoreConfig('customattribute/login/login_field'))
        {
        ...
        if (Mage::getStoreConfig('customattribute/login/login_field'))
        {
            // will try to load by attribute
            $attribute = Mage::getModel('customer/attribute')->load(Mage::getStoreConfig('customattribute/login/login_field'), 'attribute_code');
            if ($attribute->getId())
            {
    ...
    public function custom($attributeCode)
    {
        ...
        if (!$this->_customAttributes)
        {
            ...
            $alias = Mage::helper('customattribute')->getProperAlias($attributeCollection->getSelect()->getPart('from'), 'eav_attribute');
