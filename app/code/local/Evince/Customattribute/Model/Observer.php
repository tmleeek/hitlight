...
*/ 
class Evince_Customattribute_Model_Observer
{
    /**
    ...
    public function modifyOrderGrid($observer)
    {
        ...
        $alias = Mage::helper('customattribute')->getProperAlias($attributesCollection->getSelect()->getPart('from'), 'eav_attribute');
        $attributesCollection->getSelect()
            ->where($alias . 'is_user_defined = ?', 1)
            ->where($alias . 'attribute_code != ?', 'customer_activated');
            
        $alias = Mage::helper('customattribute')->getProperAlias($attributesCollection->getSelect()->getPart('from'), 'customer_eav_attribute');
        $attributesCollection->getSelect()
        ...
        if ( ($attributesCollection->getSize() > 0) && ($grid) ) {
            ...
            foreach ($attributesCollection as $attribute) {
                ...
                switch ($attribute->getFrontendInput())
                {
                    ...
                    case 'multiselectimg':
                        ...
                        $column = array(
                            ...
                            'type'         => 'options',
                            'options'      => $options,
                            'renderer'     => 'customattribute/adminhtml_renderer_multiselect',
                            'filter'       => 'customattribute/adminhtml_filter_multiselect',
                        );
                        break;
                    case 'boolean':
                        ...
                        $column = array(
                            ...
                            'type'         => 'options',
                            'options'      => $options,
                            'renderer'     => 'customattribute/adminhtml_renderer_boolean',
                        );
                        break;
                    default:
                        $column = array(
                ...
                if ('file' == $attribute->getTypeInternal()) {
                    $column['renderer'] = 'customattribute/adminhtml_renderer_file';
                }
...
//            if (Mage::helper('customattribute')->getFileAttributes('on_registration')->getSize() > 0) {
//                $html = str_replace('id="form-validate"', ' id="form-validate" enctype="multipart/form-data" ', $html);
//            }
//            if (false === strpos($html, 'customattribute')) {
//                $pos = strpos($html, '<div class="buttons-set">');
//                $insert = Mage::helper('customattribute')->fields();
...
//        }
        ...
        if ($block instanceof Mage_Customer_Block_Form_Edit) {
            if (Mage::helper('customattribute')->getFileAttributes()->getSize() > 0) {
                $html = str_replace('id="form-validate"', ' id="form-validate" enctype="multipart/form-data" ', $html);
            }
            if (false === strpos($html, 'customattribute')) {
                $pos = strpos($html, '<div class="buttons-set">');
