...
*/
class Evince_Customattribute_Adminhtml_RelationController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function indexAction()
    {
        ...
        $this->_initAction()
            ->_addContent($this->getLayout()->createBlock('customattribute/adminhtml_customer_relation'))
            ->renderLayout();
    }
    
	public function optionsAction()
    {
     	/* @var $relationModel Evince_Customerattr_Model_Relation */
        $relationModel = Mage::getModel('customattribute/relation');
    ...
    public function editAction()
    {
        ...
        if (count($attributes) == 0) {
        	$message = Mage::helper('customattribute')->__('This attribute no longer exists');
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('customattribute')->__('There is no attributes to create relations. Please create attribute first below'));
			$this->_redirect('*/adminhtml_manage/new');
			return;
        }
        
        $id = $this->getRequest()->getParam('id');
        $model = Mage::getModel('customattribute/relation');
        ...
        $details = Mage::getModel('customattribute/details')->getResourceCollection()->getByRelation($model->getId());
        Mage::register('entity_relation', $model);
        Mage::register('entity_relation_details', $details);

        $this->_initAction()
            ->_addBreadcrumb($id ? Mage::helper('customattribute')->__('Edit Relation') : Mage::helper('customattribute')->__('New Relation'), $id ? Mage::helper('catalog')->__('Edit Relation') : Mage::helper('catalog')->__('New Relation'))
            ->_addContent($this->getLayout()->createBlock('customattribute/adminhtml_customer_relation_edit'))
            ->_addLeft($this->getLayout()->createBlock('customattribute/adminhtml_customer_relation_edit_tabs'))
            ->renderLayout();
    }
    ...
    public function saveAction()
    {
        if ($data = $this->getRequest()->getPost()) {
            ...
            /* @var $model Evince_Customerattr_Model_Relation */
            $model = Mage::getModel('customattribute/relation');
            ...
            /* @var $detailsModel Evince_Customerattr_Model_Details */
            $detailsModel = Mage::getModel('customattribute/details')->getResource()->saveDetails($details);
 	...
 	public function massDeleteAction() {
        ...
        if(!is_array($relationIds)) {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('customattribute')->__('Please select item(s)'));
        } else {
            try {
                foreach ($relationIds as $relationId) {
                    $menu = Mage::getModel('customattribute/relation')->load($relationId);
                    $menu->delete();
                }
    ...
    public function deleteAction()
    {
        if ($id = $this->getRequest()->getParam('attribute_id')) {
            ...
            try {
                $model->delete();
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('customattribute')->__('Customer attribute was successfully deleted'));
                $this->_redirect('*/*/');
                return;
            }
            catch (Exception $e) {
