...
*/
class Evince_Customattribute_Adminhtml_ManageController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function indexAction()
    {
        ...
        $this->_initAction()
            ->_addContent($this->getLayout()->createBlock('customattribute/adminhtml_customer_attribute'))
            ->renderLayout();
    }
    ...
    public function editAction()
    {
        ...
        $this->_initAction()
            ->_addBreadcrumb($id ? Mage::helper('customattribute')->__('Edit Customer Attribute') : Mage::helper('customattribute')->__('New Customer Attribute'), $id ? Mage::helper('catalog')->__('Edit Product Attribute') : Mage::helper('catalog')->__('New Product Attribute'))
            ->_addContent($this->getLayout()->createBlock('customattribute/adminhtml_customer_attribute_edit')->setData('action', $this->getUrl('*/catalog_product_attribute/save')))
            ->_addLeft($this->getLayout()->createBlock('customattribute/adminhtml_customer_attribute_edit_tabs'))
            ->_addJs(
                $this->getLayout()->createBlock('adminhtml/template')
                    ->setIsPopup((bool)$this->getRequest()->getParam('popup'))
                    ->setTemplate('customattribute/attribute/js.phtml')
            )
            ->renderLayout();
    }
...
//            $model = Mage::getModel('catalog/entity_attribute');
            ...
            try {
                ...
                if ($addToSet)
                {
                    $setup     = new Mage_Eav_Model_Entity_Setup('customattribute');
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('customattribute')->__('Customer attribute was successfully saved'));
    ...
    public function deleteAction()
    {
        if ($id = $this->getRequest()->getParam('attribute_id')) {
            ...
            try {
                $model->delete();
                $relationDetails = Mage::getModel('customattribute/details')->usedInRelation($id);
                if ($relationDetails->getSize() > 0) {
                    ...
                    Mage::getModel('customattribute/details')->fastDelete($deleteRelationDetails);
                    $deleteRelationNames = array();
                    foreach ($relationIds as $relationId) {
                        if (!Mage::getModel('customattribute/details')->haveDetails($relationId)) {
                            $deleteRelationNames[] = $relationId;
                        }
                    }
                    if ($deleteRelationNames) {
                        Mage::getModel('customattribute/relation')->fastDelete($deleteRelationNames);
                    }
                    Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('customattribute')->__('Customer attribute was successfully deleted (this attribute`s relations were successfully deleted, too)'));
                } else {
