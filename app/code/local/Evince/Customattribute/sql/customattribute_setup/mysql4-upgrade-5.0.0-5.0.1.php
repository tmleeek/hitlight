...
$installer->run("
    ...
    SELECT `attribute_id`, `option_id`, `dependent_attribute_id`, `relation_id` FROM `{$this->getTable('customattribute/details')}`;
    DROP TABLE `{$this->getTable('customattribute/details')}`;
    RENAME TABLE `temp_am_customerattr_relation_details` TO `{$this->getTable('customattribute/details')}`;
");
