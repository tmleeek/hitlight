...
class Mage_Sales_Helper_Guest extends Mage_Core_Helper_Data
{
    ...
    public function loadValidOrder()
    {
        if (Mage::getSingleton('customer/session')->isLoggedIn()) {
            $source = Mage::getSingleton('core/resource');
            $read = $source->getConnection('core_read');
            $sql = '
                SELECT entity_id
                FROM `sales_flat_order`
                WHERE `sales_flat_order`.`increment_id` = ?
            ';
            $sqlQuery = $read->fetchRow($sql, array($_REQUEST['oar_order_id']));
            $urlId = 'sales/order/view/order_id/'.$sqlQuery['entity_id'];
            Mage::app()->getResponse()->setRedirect(Mage::getUrl($urlId));
        ...
        $errorMessage = 'Your request has been successfully submitted. Our customer service team will be in touch with you within one business day.';
...
//                if ((strtolower(str_replace(' ', '', trim($lastName))) != strtolower(str_replace(' ', '', trim($billingAddress->getName()))))
//                    || ($type == 'email'
...
//            }
            ...
            if (!$errors) {
                ...
                // sent email to admin
                $emailSentFrom = Mage::getStoreConfig('trans_email/ident_general/email');
                $emailNameFrom = Mage::getStoreConfig('trans_email/ident_general/name');
                $emailSentTo = Mage::getStoreConfig('cyberfision_custom/reorder/recipient_email');
                $body = "<h2>Returns and Exchanges</h2>" . '<br>';
                $body .= " Order Id: " . $_POST['oar_order_id'] . '<br>';
                $body .= " Billing Last Name: " . $_POST['oar_billing_lastname'] . '<br>';
                $body .= " Email Address: " . $_POST['oar_email'] . '<br>';
                $body .= " Reason for Return: " . $_POST['oar_reason'];
                $mail = Mage::getModel('core/email');
                $mail->setToName('Dear Admin');
                $mail->setToEmail($emailSentTo);
                $mail->setBody($body);
                $mail->setSubject('Returns and Exchanges');
                $mail->setFromEmail($emailSentFrom);
                $mail->setFromName($emailNameFrom);
                $mail->setType('html');
                $mail->send();
