...
class MageWorx_SeoBase_Model_Mysql4_Alternate_Cms_Page extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    public function getAllCmsUrls($storeIds, $forStoreId, $itemId, $isHomePage = false)
    {
        ...
        $select = $read->select()
            ...
            if ($itemId) {
                if (Mage::helper('mageworx_seobase/alternate')->getCmsPageRelationWay() == MageWorx_SeoBase_Helper_Alternate::CMS_RELATION_BY_ID) {
                    $select->where('main_table.page_id = ' . $itemId);
                }
                elseif (Mage::helper('mageworx_seobase/alternate')->getCmsPageRelationWay() == MageWorx_SeoBase_Helper_Alternate::CMS_RELATION_BY_URLKEY) {
                    $origItemId = $itemId;
                    $itemId = Mage::getSingleton('cms/page')->load($itemId)->getIdentifier();
                    $select->where("main_table.identifier ='{$itemId}'");
                }
                elseif (Mage::helper('mageworx_seobase/alternate')->getCmsPageRelationWay() == MageWorx_SeoBase_Helper_Alternate::CMS_RELATION_BY_IDENTIFIER) {
                    $origItemId = $itemId;
        ...
        foreach ($resultByPage as $result) {
            ...
            //For Individual Stores
                ...
                if (!empty($row)) {
                    ...
                    if (Mage::helper('mageworx_seobase/alternate')->getCmsPageRelationWay() == MageWorx_SeoBase_Helper_Alternate::CMS_RELATION_BY_URLKEY ||
                       Mage::helper('mageworx_seobase/alternate')->getCmsPageRelationWay() == MageWorx_SeoBase_Helper_Alternate::CMS_RELATION_BY_IDENTIFIER
                    ) {
