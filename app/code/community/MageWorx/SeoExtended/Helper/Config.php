...
class MageWorx_SeoExtended_Helper_Config extends Mage_Core_Helper_Abstract
{
    ...
    const XML_PATH_SEOSUITE_OPTIMIZED_URLS_ENABLED          = 'mageworx_seo/seoextended/optimized_urls';
    ...
    {
        if (Mage::getStoreConfigFlag(self::XML_PATH_SEOSUITE_OPTIMIZED_URLS_ENABLED, $store) && !Mage::helper('seoextended/version')->isEnterpriseSince113()) {
            return true;
        }
        return false;
    }
    ...
    {
        if (Mage::helper('seoextended/adapter_friendlyln')->isAvailableSeoFriendlyLN()) {
