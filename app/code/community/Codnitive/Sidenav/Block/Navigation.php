...
class Codnitive_Sidenav_Block_Navigation extends Mage_Catalog_Block_Navigation
{
    ...
    public function renderCategoriesMenuHtml($level = 0, $outermostItemClass = '', $childrenWrapClass = '')
    {
        ...
        foreach ($categories as $child) {
            ...
            // custom shopby project condition
            $_categoryByProject = Mage::getModel('catalog/category')->load($child->getId());
            $condition = ($paretnType == 'current') 
                    && ($child->getLevel() == $currentLevel) 
                    && ($child->getId() != $currentId);
            if ($child->getIsActive() && !$condition && $_categoryByProject->getEnableShopbyproject() !=1) {
